...
-- 실무에서는 Master Table의 데이터는 함부러 삭제를 하지 않는다
-- 필요가 없어진 데이터는 특정 칼럼에 값을 세텡하여
-- 칼럼값을 기준으로 필요한 데이터 필요없는 데이터를 구분한다

-- table에서 데이터를 삭제하기 CRUD마지막 Delete명령
-- DELETE명령도 UPDATE명령과 마찬가지로 반드시 WHERE절을 동반하는 형태로
-- 명령을 수행해야한다
-- 그렇지않으면 최소1주일간 지옥을 맛보게된다
DELETE FROM tbl_student
WHERE st_num = '10004';

-- INSERT UPDATE DELETE 명령을 수행한후 명령을 확정 짓는 명령
-- COMMIT 이후에는 ROLLBACK으로 해당 명령을 취소할수없다
COMMIT;
-- 명령 취소
-- INSERT UPDATE DELETE명령 수행 취소하는 명령
ROLLBACK;

-- 데이터를 Table 형식으로 보여줄때 모든 칼럼을 다 포함하여 보여달라
-- SELECTON Typr으로 table보기
SELECT * FROM tbl_student;

-- 데이터를 Table 형식으로 보여줄때 나열된 칼럼만 보여달라
-- projection 지정: SELECT문을 수행할때 보고자하는 칼럼들을 나열하는것
SELECT
    st_num,
    st_dept,
    st_name,
...
-- st_nume +"는" st_dept +"학과 학생입니다"
SELECT st_name||'는'||st_dept||'학과학생입니다'
FROM tbl_student
WHERE st_dept = '음악과';

--SELECT 문 을 이용한 4칙연산 구현
...
DESC tbl_student;
 ...
 -- VARCHAR2: 가변문자열, 데이터의길이가 일정하지 않는칼럼
 -- 크기를 지정할때 가장 길이가 긴 데이터를 기준으로 감아서 정한다
 -- NVARCHAR2 : 가변문자열 유니코드문자열을 수용하는칼럼
...
SELECT * FROM tbl_student
WHERE st_dept ='컴퓨터공학';
-- 학과가 콤퓨터 공학과 이고(AND)3학년인 학생들만 보여달라
SELECT * FROM tbl_student
WHERE st_dept ='컴퓨터공학' AND st_grade = 3;
-- 학과가 컴퓨터 공학이거나(OR) 학과가 법학인 학생들만 보여라
SELECT * FROM tbl_student
WHERE st_dept ='컴퓨터공학' OR st_dept ='법학';

-- 학과가 컴퓨터 공학 또는 법학인 학생들의 정보중 이름과 학과만 나열하라
-- SELECTION으로 레코드를 제한하고 
SELECT st_name, st_dept FROM tbl_student
WHERE st_dept ='컴퓨터공학' OR st_dept ='법학';
