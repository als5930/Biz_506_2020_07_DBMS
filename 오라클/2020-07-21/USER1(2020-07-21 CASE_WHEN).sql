...
-- 기존의 PK가 있으면 중복이 생성되지 않는다
-- PK_iolist :오라클에서만 사용되는 PK를 찾는 별명
-- 혹시 PK를 삭제할 일이 있을때 필요한 이름
ALTER TABLE tbl_ioilist ADD CONSTRAINT PK_iolist PRIMARY KEY (io_seq,io_date);
ALTER TABLE tbl_ioilist DROP PRIMARY KEY;


-- 오라클에는 TABLSE에 AUTO_INCREMENT 설정이 없다
-- 만약 일련번호와 같이 칼럼을 사용하여 PK오 만들고 싶을때는 상당히 곤란한 상황이 된다
-- INSERT를 수행할때마다 이전에 저장된 일련번호값을 조회하고 +1을 수행하여 다시 INSERT 수행하는 일을
-- 반복해야 라기 때문에

-- 다행히 오라클에는 SEQUENCE라는 다른 DBMS에 없는 특별한 객체가 있다
-- 이객체는 객체.NEXTVAL 이라는 특별한 호출자가 있는데
-- 이호출자는 호출할때마다(SELECT,INESRT등에서)내부에 가지고 있는 변수 값을
-- 사전에 정의한 규칙에 따라 자동 증가하여 보관을 한다
-- 이객체.NEXTVAL 이 가지고 있는 값을
-- INSERT를 수행할때 PK인 seq칼럼에 주입을하면 AUTO_INCREMENT 효과를 낼수있다 
