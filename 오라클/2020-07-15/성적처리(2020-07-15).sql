...
-- 각과목별 총점을 계산하기 ==> 집계 통계를 수행한다
SELECT SUM(sc_kor) AS 국어점수,
    SUM(sc_eng) AS 영어점수,
    SUM(sc_math) AS 수학점수,
    SUM(sc_music) AS 음악점수,
    SUM(sc_art) AS 미술점수
FROM tbl_score;

SELECT AVG(sc_kor) AS 국어점수,
    AVG(sc_eng) AS 영어점수,
    AVG(sc_math) AS 수학점수,
    AVG(sc_music) AS 음악점수,
    AVG(sc_art) AS 미술점수
FROM tbl_score;

--SELECTION 적용하여 구현
-- 학번이 20001~20010까지 학생의 리스트만 보고 싶을때
...
-- 의미없는(DUMY) 칼럼을 '==과목초점==' 이라는 값으로 만들었다
-- 통계, 집계합수를 사용할때 통계,집계함수로 묶이지 않은 칼럼은 
