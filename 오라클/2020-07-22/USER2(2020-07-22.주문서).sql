...
-- 이러한 코드가 필요할떄 시스템에 이미 준비되어 있는 DUAL이라는 DUMMY 테이블을 사용해서 코딩을 한다.
...
-- 지금 tbl_order 테이블에 위와 같은 데이터가 있을떄
-- O00001       C0032       P00001 이러한 데이터를 추가한다면 아무런 제약없이 값이 추가되 버릴 것이다.
...
-- unique를 추가하는데 이미 uniqe 조건에 위배는 값이 있으면 제약조건이 추가되지 안흔ㄴ다
-- duplicate keys found : pk uniqe를 설정된 칼럼에 값을 추가하거나
-- 이미 중복된 값이 잇는데 pk; d유니크를 설정하려 할때 발생을 한다.
-- unique 제약조선을 추가하기 위해 마지막에 추가된 데이터를 제거하려고 다음과 같은 sql을 실행한다.
...
-- 복수의 칼럼으로 pk를 설정하는데
-- UPDATE, DELETE 수행할때 WHERE 칼럼 1 = 값 AND 칼럼2 = 값 AND 칼럼3= 값 .... 과 같은 조건을 부여해야한다.
-- 이것은 데이터 무결성을 유지하는데 매우 좋지않은 한경이다.
