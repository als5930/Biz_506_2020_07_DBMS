...
--TBL_STUDENT 데이터 중애서 학과가 어떠한 학과들이 있는지 알고싶다
-- 학과이름이 같은이름들이 다수 존재하는데 이들 이름을 1개씩만 추출하여
-- 리스트로 보여준다면 어떠한 학과 있는지 알기 쉬울것 같다
-- st_dept 칼럼에 저장된 값들을 같인 값끼리 그룹을 지어라
-- 그 그룹지은 이름을1개씩만 나열해 달라
...
-- st_name 칼럼에 저장된 값들이 중복된 값이 없어서 GROUP BY 가 의미 없는경우도있다
SELECT st_name FROM tbl_student
GROUP BY st_name;

--데이터를 학과이름으로 그룹을 짖고
-- 각 학과에 소속된 학생들이 몇명씩인가를 알고싶다
...
-- 각 학과 별로 소속된 학생들의 나이합계가 알고싶다
SELECT st_dept,SUM(st_age) FROM tbl_student
GROUP BY st_dept;

-- 각 학과 별로 소속된 학생들의 평균 나이가 알고싶다
SELECT st_dept,AVG(st_age) FROM tbl_student
GROUP BY st_dept;


-- 각 학과의 학생들 중에 나이가 가장 많은 학생의 나이와
-- 나이가 가장 적은 학생의 나이를 보여달라
--단 가장나이가 많은 학생의 이름, 또는 가장 나이가 적은 학생의 이름을 찾는데는
-- SQL로는 어렵다
