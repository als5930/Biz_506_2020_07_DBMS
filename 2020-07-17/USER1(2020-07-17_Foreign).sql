...
-- 테이블에 추가되는 데이터의 무결성을 보증하는 제약 조건 설정
-- JOIN 사용하여 두개의 테이블에 Replation을 설정하기도 하는데
-- 그 차이점
-- JOIN은 단순히 Viwe의 편리함(필요한 정보를 Projectuin포함)을 위해서 사용하기
...
-- 이런경우 초기에 FK를 설정하디 않거나 FK 제거하고 데이터를 입력(추가)하도록
-- 하는 경우가 많다

-- 현재 스키마에 있는 학생정보와 성적정보 테이블을 FK(외래키) 관계로 설정을 한다
-- FK 관계의 테이블을 부모 테이블과 자식테이블로 분류하는데 
-- FK 설정은 자식테이블에서 실시한다 
...
REFERENCES tbl_student(st_num); -- 부모는 누군데 : TBL_STUDENT,
                                --연동(연결)할부모 칼럼은 어떤것인데 : tbl_st_num
...
-- 학생정보에 없는 20200 학생의 국어 성적을 추가 했다
--20200 학생의 국어점수가 학적정보가 이상이없는 학생의 성적인지
-- 확신할수있는방법이 없다
--따라서 이데이터는 무결성원칙에 위배되는 데이터 일수있다
...
-- 만약 학생 정보가 데이터가 필요가 없어서 삭제를 할경우
--  먼저 성적정보에서 삭제하고자 하는 학번에 성적데이터를 모두 삭제해야한다

-- 아주특별하게 학생의 학번(st_num)변경할 경우
--  1.먼저 성적정보에서 변경할 학번의 성적데이터를 모두 삭제하고
--  2. 학생의 학번을 변경하고
--  3. 변경된 학번으로 성적데이터를 추가해야한다
-- ALTER TABLE 명령을 수행하게 되면 명령이 시작된 순간 table의 모든 데이터는 접근이 금지된다
-- 만약 저장된 데이터가 매우 많다면
-- 상당한 시간이 소요될 것이고, 그동안 table의 데이터를 필요로 하는
-- App은 실행이 중단된다
-- 칼럼추가 ,칼럼삭제, 칼럼명변경, 칼럼타입변경 등을 수행할수있다
-- 칼럼을 삭제, 변경하는 경우 기존 데이터가 손상되는 위험이 있다
...
-- 자식테이블의 모든 데이터를 찾아서 같은 값을 바꿔라
--ON UPDATE CASCADE;

-- FK 를 설정하는 대상
-- 테이블간의 데이터가 1:N 인경우 N상태의 테이블에 설정을 한다
--      REFERRENS : 1상태의 테이블
-- 테이블간의 데이터가 1:1 인경우 Work Table에설정을 한다
--      REFERRENS : Master Table에설정을 한다

-- 학생정보, 학과 정보간에 FK 설정하려고 한다
-- 학과정보는 학과명(코드)이 유일한 값으로 정해져 있다 같은 이름의 학과는 존재할수 없다
-- 학생정보에는 학생마다 학과가 다르거나 같을수 있어서 수없이 많은 중복데이터가 있다
-- 부모테이블 : 학과 1인 경우
-- 자식테이블 : 학생정보중의 학과는 N 인경우가 된다
ALTER TABLE tbl_student
ADD CONSTRAINT FK_D_ST --한스키마내에서는 이름을 유일해야 한다
FOREIGN KEY (st_dept)
