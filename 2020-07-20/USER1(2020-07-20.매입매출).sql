...
-- 최근 App을 개발하여 데이터베이스 화 하고 관리를 수행하려 한다
-- 엑셀 데이터를 받아서 DB로 보내기 위해 변환 작업을 수행하고 있다.

-- 엑셀 매입매출원장을 DB로만들기 위해 매입, 매출을 분리하고, 날짜를 문자열화 했다
-- 이데이터를 DB에 저장ㄹ하려고 봤더니, 이 데이터에 PK칼럼으로 저장할 만한 칼럼을 찾을수 없다
-- 이러한 경우(Work DB 경우)에는 실제 데이터와 별도로 임의의 pk칼럼을 생성해서 관리해주는 것이 좋다
...
-- 데이터를 확인하다 봤더니 단다와 금액이 0인 데이터 보인다
SELECT * FROM tbl_iolist
WHERE io_price = 0 OR io_amt = 0;

--투니스, 7+8칫솔, 레종블루
-- 이 3가지 데이터가 거래된 5,10,28 SEQ 데이터가 칼럼값이 0이다
-- 이 데이터의 단가, 금액을 수정하기 위해 동일한 상품 데이터가 있는지 확인 작업
SELECT * FROM tbl_iolist
WHERE io_pname IN('투니스', '7+8칫솔', '레종블루');
-- 검색을 해 봤더니 위3가지 상품의 거래 내역이 다른것이 없다
-- 다른곳에서 찾어서 값을 입력해야 할 것이다
-- 투니스 :1000, 칫솔 : 2500, 레봉블루:4500으로 변경하자

-- 투니스 거래 내용의 단가, 금액을 변경할텐데
-- WHERE io_pname ='투니스' 와 같이 지정하면
-- 혹시 다른 매입, 매출 데이터가 있을경우 막대한 문제를 일으킬수 있다
-- 반그시 PK칼럼값을 조회하고 PK 칼럼으로 WHERE 절을 사용하자
...
-- 그러한 불편을 막기 위해 ip_inout 칼럼의 조건으로 하는 DECODE 함수를 사용하여
-- 단가 부분을 매입단가, 매출단가로 분리하여 보여주도록 Query를 작성
...
SELECT io_dname, io_inout,io_qty,
    ...
    DECODE(io_inout,'매입', io_price,0) AS 매출단가,
    DECODE(io_inout,'매출',io_amt,0) AS 매입합계,
    DECODE(io_inout,'매출',io_amt,0) AS 매출합계  
FROM tbl_iolist
WHERE (io_seq > 0 AND io_seq <= 10) OR (io_seq >= 300 AND io_seq <=310)
ORDER BY io_dname;

-- '거래처별로' 매입과 매출을 구분하여 볼수 있는 쿼리를 완성했다
-- 거래처별로 매입과 매출의 합계를 계산해보고싶다 
-- 이 데이터(일년간 거래내역)에서 거래별로 총 매입금액, 매출금액 계산을 해보고싶다

--1. '거래처별로' 거래내역이 여러번 반복이루어 졌으므로 거래처명이 여러번 나타날 것이다
-- 이데이터를 거래처별로 묶어야한다
-- GROUP BY io_dname
-- 2. 전체 매출금액과 매입금액을 표시하는 가상칼럼(매출금액 매입금액을계산한)을
-- 통계합수로 묶어준다
