...
public class OrderInput {
	...
	public boolean orderInsert() {
		...
		while (true) {
			...
			try {
				...
				System.out.println("가격은 숫자로만 입력가능");
				continue;
			}
	...
	public boolean orderUpdate() {
		...
		// 여기까지 정상진행이 되면 입력한 SEQ에 해당하는 데이터가 orderVO에 담겨 있다.
		System.out.printf("변경할 주문번호 : (%s, Enter : 변경안함, QUIT : 종료) >> ", orderVO.getO_num());
		String str_num = scan.nextLine();
		if (str_num.equals("QUIT")) {
			return false;
		}
		// 변경할 주문번호를 입력했을때는 입력한 주문번호를 orderVO의 o_num에 세팅한다.
	...
	public boolean orderDelete() {
		...
		System.out.println("삭제할 SEQ(QUIT:종료)>>");
		String strSeq = scan.nextLine();
		if (strSeq.equals("QUIT")) {
			return false;
		}
		...
		try {
			...
			if (orderVO == null) {
				...
				System.out.println("주문번호: %s, 고객번호: %s" + "상품코드: %s 의 주문을 삭제할까요?(YES:삭제 )");
				orderVO.getO_num();
				orderVO.getO_cnum();
				orderVO.getO_pcode();
