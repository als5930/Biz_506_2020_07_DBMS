...
public class OrderServiceImplv1 implements OrderService {
	...
	/*
	 * thorws Exception try-catch를 사용하여 자체적으로 exception을 Handdling(처리)를 하였는데
	 * try-catch가 코드내에 많아지면 코드의 가독성이 매우 떨어진다.
	 * 
	 * exception 처리를 직접하지 않고 이 method를 호출한곳에 대리 요청한다(exception을 던진다)
	 * 
	 */

	/*
	 * insert를 호출하는 곳에서는 OrderVO에 주문정보 데이터를 담고 매개변수로 전달해야 하며 try-catch로 exception처리를
	 * 하고 결과값으로 정수값을 받아서 insert 유무를 처리해야 한다.
	 */
	@Override
	...
	/*
	 * selectAll()과 findBySeq() method에서 ResultSet으로 부터 데이터를 getter하여 OrderVO에
	 * setter하는 부분을 별도 method로 분리하고 selectAll()과 findBySeq()에서 호출하여 사용할수 있도록 구현
	 */
