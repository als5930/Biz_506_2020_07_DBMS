 ...
 *  DBMS에 SQL을 보내고, 받을때 SQL을 DBMS가 알수있는 데이터러 변환하고
 *  DBMS가 데이터(Table)를 보낼때 Java가 인식할수 있는 데이터로 변환
 *  
...
public class DBConnection {
	...
	static {
		...
		try {
			...
			// mybatis cofiguration 문서를 읽어서 xml로 설정된 항목들을 가져와서 mybatis에서 사용할수 있는
			...
			// inputStream 객체를
			// 매개변수로만 전달하면 sqlSessionFactory를 만들어서 return 해준다

			// sqlSessionFactory는 프로젝트가 시작될때 만둘어져 JVM이 관리하는 객체가 된다(static 이기때문에)
			// 만약 이 프로젝트를 어떤경로로 2번이상 작동을 시킨다면
			// 작동된 회수만큼 sqlSessionFactory가 계속해서 생성될 것이다
			// sqlSessionFactory 객체는 하는일이 꽤 많아서 생성하는 것 자체만으로도
			// JVM입장에서는 부담이 된수 있다
			// sqlSessionFactory를 생성하기 전에 누군가 만들어서 사용중이라면 다시 만들지 않고 재활용을 할수 있도록 해주는 것이 좋다
			// if(sql... ==null): 아직 아무것도 sqlSessionFactory를 만들지 않았을때만
			// 생성하도록 조건을 부여하는 것이다
