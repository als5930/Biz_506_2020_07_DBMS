...
-- 이상 현상
-- 추가 이상, 변경(수정) 이상, 삭제 이상
-- DBMS에 저장하는 저장된 데이터를 관리할때(SELECT 외의 CUD에서) 문제가
-- 발생하여 원본데이터가 무결성을 잃는 상황
-- 추가이상 방지 : PK, UNIQUE, NOT NULL 이러한 제약조건을 설정하여 잘못된 데이터가
--     INSERT되는 현상을 막기
...
-- 변경 삭제 이상을 방지 : 특별한 경우가 아니면 2개 이상의 레코드에 변화가 발생하지
--                          않도록 코드나 관리를 설계해야 한다.                    
...
-- SELECT TABLE 에 Al
---------------------------------------------
-- 다수의 table을 JOIN으로 설정하다 보면 다른 table의 같은 칼럼이름이 존재할 수 있다.
-- 보통은 칼럼에 이니셜을 prefix를 붙여 그 구분을 명확하게 하지만
-- table이 많아지다보면 같은 이름의 칼럼이 존재할 수 있다.
-- 이러한 상황에서 JOIN을 수행하면 칼럼을 제대로 인식하지 못하여 오류가 발생하는 경우가 있다.
-- (실제로 많다)
-- 이런경우 table에 Alias를 설정해주면 오류를 막을 수 있다.
-- [table] AS [alias]형식으로 작성
-- 단, Oracle에서는 AS 키워드를 사용하지 않는다.
SELECT tbl_student.st_num, ST.st_name,
       ST.st_dept, DT.d_name,
       DT.d_prof, DT.d_tel,st_grade,
       ST.st_tel, ST.st_addr
FROM tbl_student ST
...
WHERE d_name = '법학';
 ...
 -- SELECT명령문을 사용하여 복잡한 QUERY를 작성하고
 -- 작성된 Query를 자주 사용하게 될 것으로 예상이 되면
 -- 이 Query를 View 생성해 보관할수 있다
 
 -- View는 실제 Table과 똑같이 SELECT 명령을 통해 데이터를 조회할수 있다
 -- 하지만 View는 실ㄹ제 데이터를 가지고 있지 않다
...
--중요한정보를 봐서는 안되는 사용자가 있을때
-- 보여줘도 문제 없는 칼럼만 PROJECTION한 쿼리를 만들고
-- 이쿼리를 VIEW생성해 두면
-- 권한이 제한된 사용자는 꼭필요한 정보만을 보게되어
-- 보안, 개인정보 보호 등의 용도로 사용 할수있다
